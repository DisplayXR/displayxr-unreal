...
### Fixed
- **The plugin submitted the active rendering mode's tile count as the OpenXR view count, which a conformant runtime now rejects.** `GetDesiredNumberOfViews()` returned `max(TileColumns * TileRows, 2)` and the compositor's `XrCompositionLayerProjection::viewCount` followed it, so the render loop was N-wide even though the plugin's *content* has only ever been two views. DisplayXR runtime `main` makes `XR_VIEW_CONFIGURATION_TYPE_PRIMARY_STEREO` — the type this plugin begins its session with — report exactly 2 views and makes `xrEndFrame` refuse a projection layer with `viewCount > 2` under it (`XR_ERROR_VALIDATION_FAILURE`). On a display whose active mode has more than two tiles the plugin would therefore have had every frame rejected. The submitted view count is now clamped to 2 (`DisplayXRMaxSubmittedViews`), while the **tile geometry** still comes from the mode, so `AdjustViewRect` paints tiles 0 and 1 of that mode's grid and the runtime's under-submit contract composites the frame. Nothing changes on any display that ships — every one of them reports a 1x1 (2D) or 2x1 (3D) mode, where the two counts are equal. See [the runtime's view-configuration model](https://github.com/DisplayXR/displayxr-runtime/blob/main/docs/reference/view-configuration-model.md).
- **The IPC array-copy path could write past the end of the swapchain array.** The same tile count drove `CopyInfo.DestSliceIndex` in `ReleaseImage_RT` while the array swapchain is created with `arraySize = 2`, so a 4-tile mode would have issued copies to slices 2 and 3 of a two-slice texture. The copy loop and the swapchain's `arraySize` now share one constant.
- The instanced-stereo layout warning fired on layouts ISR can in fact render. With exactly two views submitted, any mode with **two or more columns** puts both of them side by side at `Y = 0` — what ISR draws. The warning is now scoped to the one layout that still breaks it: a single-column, multi-row (top/bottom) mode.
- Release tooling: the packaged binaries' Authenticode check is now **fail-closed** and prefers PowerShell 7 (`pwsh`). Windows PowerShell 5.1 launched from git-bash on a box that also has pwsh inherits pwsh's `PSModulePath`, cannot load `Microsoft.PowerShell.Security`, and the old "count the non-Valid DLLs" test read that errored pipeline as zero failures — reporting `SIGNED=yes` for the v0.9.3 run on an unproven package. The check now counts *successes* against the total and reports `unverified` unless every DLL comes back Valid.

### Changed
- **`FDisplayXRViewConfig` splits its two counts.** `GetViewCount()` is replaced by `GetTileCount()` (atlas grid geometry: atlas sizing, view-rect placement, copy offsets) and `GetSubmittedViewCount()` (the OpenXR view count: projection views, `CachedViews`, array slices, clamped to `DisplayXRMaxSubmittedViews`). Conflating the two is what the fixes above were. Private plugin header; no public API change.
- **Runtime ABI pin bumped to `displayxr-runtime@c1e4fe00d`.** Re-vendored `XR_DXR_display_info.h` (spec v16 → v19) and `XR_DXR_view_rig.h` (comment only). The `display_info` diff is purely additive: the `XR_VIEW_CONFIGURATION_TYPE_PRIMARY_MULTIVIEW_DXR` enum value, `XrDisplayDesktopInfoDXR` (v18) and `XrEventDataDisplayModeRequestDeniedDXR` (v17). No existing struct changed size or layout, and the plugin uses none of the new members yet. **This release requires DisplayXR runtime v2.17.0 or newer.**

Verified on the Windows box against runtime v2.17.1: exactly 2 views located and submitted in every mode, a Quad (2x2) mode accepted both in-process and on the IPC path, and zero `XR_ERROR_VALIDATION_FAILURE` (DisplayXR/displayxr-unreal#59).

## [0.9.3] - 2026-09-17

### Changed
- Removed the dead off-axis projection math and the docs that still named it (the runtime owns the view math via `XR_DXR_view_rig`); `Docs/DisplayXR/InstancedStereo.md` gains *Culling under ISR*, *The Nanite single-pass velocity bug* and a list of other secondary-eye engine issues found in 5.7.4.

### Fixed
- **Moving Nanite meshes shimmered in the secondary eye under instanced stereo.** Not a plugin defect: with `r.Nanite.MultipleSceneViewsInOnePass=1` (UE's default) Nanite draws both scene views in one pass, so `Nanite::EmitDepthTargets` runs the depth/velocity export once over the whole family rect (`DumpGPU` shows the pass `Emit Scene Depth/Resolve/Velocity` with `ViewRect = (0, 0, 3840, 1080)`, scope `View0 (together with 1 more)`). `CalculateNaniteVelocity` reconstructs each pixel with the Common.ush helpers `SvPositionToWorld()` / `SvPositionToScreenPosition()`, which read the **primary** view's uniform buffer, so secondary-eye pixels are converted in the primary eye's viewport and get garbage motion vectors — the right half of `SceneVelocity` comes out several times larger than the left and worsens towards the right edge, while the ordinary `VelocityParallel` pass in the same frame stays symmetric. Every temporal consumer (TSR, Lumen reflections, SSR, motion blur) then shimmers on moving Nanite meshes in the right eye, while static geometry (no `PRIMITIVE_SCENE_DATA_FLAG_OUTPUT_VELOCITY`) is clean. `FDisplayXRDevice` now also forces `r.Nanite.MultipleSceneViewsInOnePass=0` when it detects instanced stereo, under the existing `r.DisplayXR.InstancedStereoWorkarounds` opt-out; instanced stereo stays enabled and only Nanite gives up its single-pass saving. The cvar is gated on `View.bIsMultiViewportEnabled`, so this changes nothing for non-stereo rendering or projects without Nanite. See `Docs/DisplayXR/InstancedStereo.md`.
- **Instanced stereo culled both eyes with a frustum that did not contain them.** Under single-pass stereo the engine builds one culling frustum for the pair from the device's `eSSE_MONOSCOPIC` answers. `CalculateStereoViewOffset` ignored that index and `GetStereoProjectionMatrix` returned an angle-wise union of the eye fovs, so the culling apex stayed at the camera while the eye frusta have their own apexes (metres behind the camera on a display-centric rig, moving every frame on any rig); edge content, lights and reflection captures could be culled for the pair while the secondary eye still needed them. When instanced stereo is compiled in, the mono view now sits at the rearmost eye depth and the eyes' mean lateral position and is widened so every eye's edge ray stays inside from its own near plane (`ComputeMonoView`). Multi-pass stereo is unchanged. On the test project's display-centric rig this was the difference between a broken ISR image (checker materials, black shadow slab, no lighting) and a correct one. Docs: `Docs/DisplayXR/InstancedStereo.md` gains *Culling under ISR* and a list of secondary-eye engine issues found in 5.7.4 (Lumen front-layer translucency uniform buffer, `ViewRectMinAndSize`).
